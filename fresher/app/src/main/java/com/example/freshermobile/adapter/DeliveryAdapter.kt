...
class DeliveryAdapter(private val listItem: MutableList<DeliveryModel>): RecyclerView.Adapter<DeliveryAdapter.DeliveryHolder>() {
    ...
    inner class DeliveryHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        private fun enterEditText(item: DeliveryModel.EnterEditText) {
            ...
            if (item.moreAction != null) {
                itemView.findViewById<ImageButton>(R.id.button_more_action).setImageResource(item.moreAction)
            }else{
